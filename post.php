...
<?php
	...
	$lazypic=$this->options->themeUrl.'/src/lazy.svg';
	$titleflag = 1;
/*	if($this->fields->src == 0) {
	...
	foreach($imgs as $img) {
		echo '<div class="post-item col-xs-6 col-sm-4 col-md-2" data-src="'.$img['url'].'"><img src="'.$lazypic.'" title="'.($this->fields->title == 1 ? $img['name'] : ($this->title.' ['.$titleflag++.']')).'" class="post-item-img" data-original="'.$img['url'].'"></div>';
	}
?>
	</div>
	<div class="post-info">
